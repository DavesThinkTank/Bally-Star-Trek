...
Bally Star Trek: Arduino Version
...
// From the description to the right of each variable, determine the value that represents your system. Type that 
// value to the right of the variable name.

#define KOBAYASHI_MARU_TIME 15       // Time in seconds to disable a Klingon during Kobayashi Maru test. 15 is recommended.
#define NACELLE_PATTERN 2            // Pattern for nacelle lights for most of the game. 0, 1, 2 indicates off, bright, or dimmed.
...
// TELL THE ARDUINO IF YOU HAVE OR DO NOT HAVE MUSIC FILES SET UP ON THE WAV TRIGGER! 
// Complete the following statement with either the word true or false:
#define I_HAVE_ADDED_MUSIC_FILES true

#define VERSION_NUMBER 2026.06       // Version number appears in displays #1 and #2 at start of game
#define ATTRACT_SPEECH_TIMER 300000  // Amount of time between attract mode speech callouts. 60000 = 1 minute
...
// Other functions needing to be defined:

boolean BALLYScored(unsigned long delayPT = 0, boolean selfTest = 0);
...
unsigned long resumeMusic = 0xFFFF;
...
byte switchHit;
int numSwitchHits = 0;
unsigned long switchHitVals[120] = { 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99, 99,
...
unsigned long ScoreAward1[4] = { 0, 0, 0, 0 };
unsigned long ScoreAward2[4] = { 0, 0, 0, 0 };
unsigned long ScoreAward3[4] = { 0, 0, 0, 0 };
unsigned long PersonalGoal[4] = { 0, 0, 0, 0 };
...
byte DestructTimer = 0;
byte FinalTime = 0;
boolean hiding = true; // Mini game 3 value. True if hiding, false if running.
boolean holdhiding = hiding;
boolean dropsDown = false;
int KlingonCount = 0;

unsigned long BallSearchTimer = 0;
byte BallSearchStatus = 0;
boolean TiltIgnore = 0;
byte gamePlayed = 0;

byte GameMode = 1;
byte holdGameMode = 1;
unsigned long LongPressTimer = 0xFFFFFFFF;
...
boolean EELightShow = 0;
boolean WinnerLightShow = 0;
boolean MatchLightShow = 0;
boolean AttractLightShow = 0;
...
unsigned long HighScore[4] = { 0, 0, 0, 0 };
unsigned long HighScoreBeat = 0;

signed long ExtSavedValue = 0;
unsigned long ExtResetHold = 0;
unsigned long ExtLastResetPress = 0;
unsigned long ExtLastOtherPress = 0;
unsigned long ExtNextSpeedyValueChange = 0;
unsigned long ExtNumSpeedyChanges = 0;
byte dispNum = 0;
int UpDown = 1;
...
void SetupDIPVars() {  // Set up all variables derived from the DIP switches
  ...
  dipThresholdAward = dipThresholdAward + (dipThresholdAward < 2);  // 1-2-3 = No reward - extra ball - free game
...
void setup() {
  ...
  RPU_SetDisplay(1, 100 * (VERSION_NUMBER + 0.005 - floor(VERSION_NUMBER)), true, 2);
  RPU_SetDisplay(2, RPU_OS_MAJOR_VERSION, true, 2);
  RPU_SetDisplay(3, RPU_OS_MINOR_VERSION, true, 2);
  ...
  if (ScoreAward1[1] > 10000000 || ScoreAward1[2] > 10000000 || ScoreAward1[3] > 10000000) {  // Likely first time running program since update! Set defaults.
    RPU_WriteULToEEProm(RPU_HIGHSCORE_EEPROM_START_BYTE_2, HighScore[1] = 50000);  // Kid's Mode
    RPU_WriteULToEEProm(RPU_HIGHSCORE_EEPROM_START_BYTE_3, HighScore[2] = 100000); // Challenge Mode
    RPU_WriteULToEEProm(RPU_HIGHSCORE_EEPROM_START_BYTE_4, HighScore[3] = 100000); // Future Mode
    RPU_WriteULToEEProm(RPU_AWARD_SCORE_1_EEPROM_START_BYTE_2, ScoreAward1[1] = 25000);
    RPU_WriteULToEEProm(RPU_AWARD_SCORE_1_EEPROM_START_BYTE_3, ScoreAward1[2] = 0);
    RPU_WriteULToEEProm(RPU_AWARD_SCORE_1_EEPROM_START_BYTE_4, ScoreAward1[3] = 0);
    RPU_WriteULToEEProm(RPU_AWARD_SCORE_2_EEPROM_START_BYTE_2, ScoreAward2[1] = 75000);
    RPU_WriteULToEEProm(RPU_AWARD_SCORE_2_EEPROM_START_BYTE_3, ScoreAward2[2] = 0);
    RPU_WriteULToEEProm(RPU_AWARD_SCORE_2_EEPROM_START_BYTE_4, ScoreAward2[3] = 0);
    RPU_WriteULToEEProm(RPU_AWARD_SCORE_3_EEPROM_START_BYTE_2, ScoreAward3[1] = 150000);
    RPU_WriteULToEEProm(RPU_AWARD_SCORE_3_EEPROM_START_BYTE_3, ScoreAward3[2] = 0);
    RPU_WriteULToEEProm(RPU_AWARD_SCORE_3_EEPROM_START_BYTE_4, ScoreAward3[3] = 0);
    RPU_WriteULToEEProm(RPU_PERSONAL_GOAL_EEPROM_START_BYTE_2, PersonalGoal[1] = 75000);
    RPU_WriteULToEEProm(RPU_PERSONAL_GOAL_EEPROM_START_BYTE_3, PersonalGoal[2] = 150000);
    RPU_WriteULToEEProm(RPU_PERSONAL_GOAL_EEPROM_START_BYTE_4, PersonalGoal[3] = 150000);
  }
...
void ReadAllEEProm() {
  ...
  ScoreAward1[0] = RPU_ReadULFromEEProm(RPU_AWARD_SCORE_1_EEPROM_START_BYTE, 0);        // 600,000
  ScoreAward2[0] = RPU_ReadULFromEEProm(RPU_AWARD_SCORE_2_EEPROM_START_BYTE, 0);        // 1,200,000
  ScoreAward3[0] = RPU_ReadULFromEEProm(RPU_AWARD_SCORE_3_EEPROM_START_BYTE, 0);        // 2,800,000
  PersonalGoal[0] = RPU_ReadULFromEEProm(RPU_PERSONAL_GOAL_EEPROM_START_BYTE, 100000);  // 500,000
  HighScore[0] = RPU_ReadULFromEEProm(RPU_HIGHSCORE_EEPROM_START_BYTE, 1000);

  for (byte i = 1; i < 4; i++) {
    ScoreAward1[i] = RPU_ReadULFromEEProm(RPU_AWARD_SCORE_1_EEPROM_START_BYTE_2 + (i - 1) * 20, 0);
    ScoreAward2[i] = RPU_ReadULFromEEProm(RPU_AWARD_SCORE_2_EEPROM_START_BYTE_2 + (i - 1) * 20, 0);
    ScoreAward3[i] = RPU_ReadULFromEEProm(RPU_AWARD_SCORE_3_EEPROM_START_BYTE_2 + (i - 1) * 20, 0);
    PersonalGoal[i] = RPU_ReadULFromEEProm(RPU_PERSONAL_GOAL_EEPROM_START_BYTE_2 + (i - 1) * 20, 100000);
    HighScore[i] = RPU_ReadULFromEEProm(RPU_HIGHSCORE_EEPROM_START_BYTE_2 + (i - 1) * 20, 100000);
  }
...
boolean AddPlayer(boolean resetNumPlayers = false) {
  ...
  if (!FreePlay) {
    Credits -= 1;
    PlaySound(SFXAddPlayer);  // Transporter
    RPU_WriteByteToEEProm(RPU_CREDITS_EEPROM_BYTE, Credits);
    RPU_SetDisplayCredits(Credits, dipCreditsDisplayed);
    RPU_SetLampState(LA_CREDIT_INDICATOR, Credits > 0);
    RPU_SetCoinLockout((Credits < dipMaxCredits) ? false : true);
...
// #################### BAD SWITCHES ####################
boolean BadSwitches() {  // An occasional error in the Arduino causes all switches to fire at once! This code is meant to catch these errors before they can disrupt the game.

  byte num = RPU_SwitchCount();

  if (num < 4) return false;  // number of switches allowed to be hit at once.

  RPU_ClearSwitches();

  // Options to alert user
  PlaySound(SFXError[2]);  // Error message played. The old game's Star Trek theme.
  RPU_SetDisplayCredits(99);
  RPU_SetDisplayBallInPlay(num);
  return true;
}
...
// #################### CHECK FOR PLAYFIELD SWITCHES ####################
boolean CheckforPlayfieldSwitch(byte switchHit) {
  // Used together with BallSearchRoutine to determine if ball search required

  switch (switchHit) {
    case SW_CREDIT_BUTTON:       // Non-playfield switches do not reset ball search
    case SW_COIN_1:
    case SW_COIN_2:
    case SW_COIN_3:
      return false;
    
    case SW_TILT:                // Switches which do not reset ball search, and also do not count during tiltignore.
    case SW_SLAM:
    case SW_HYPER_SPACE_LOWER:   // Ball can get stuck on these two switches, repeatedly firing them due to vibration or tilt
    case SW_HYPER_SPACE_UPPER:  
      if (TiltIgnore || BallSearchStatus != 0) return true;
      return false;
  }
  BallSearchTimer = CurrentTime; // Reset ball search
  BallSearchStatus = 0;
  if (TiltIgnore) {
    TiltIgnore = 0;
...
// #################### BALL SEARCH ROUTINE ####################
void BallSearchRoutine() {
  switch (BallSearchStatus) {
    case 0: 
      if (CurrentTime - BallSearchTimer > 20000) { // Wait 20 seconds without a playfield switch hit before conducting ball search
        BallSearchStatus += 1;
        BallSearchTimer = CurrentTime;
        My_PushToSolenoidStack(SO_SAUCER, 5, true);
      }
      return;
    case 1: 
      if (CurrentTime - BallSearchTimer > 250) {   // Conduct ball search: pop all solenoids, stop if playfield. switch hit
        BallSearchStatus += 1;
        BallSearchTimer = CurrentTime;
        My_PushToSolenoidStack(SO_POP_LEFT, 5, true);
      }
      return;
    case 2: 
      if (CurrentTime - BallSearchTimer > 250) {
        BallSearchStatus += 1;
        BallSearchTimer = CurrentTime;
        My_PushToSolenoidStack(SO_POP_RIGHT, 5, true);
      }
      return;
    case 3: 
      if (CurrentTime - BallSearchTimer > 250) {
        BallSearchStatus += 1;
        BallSearchTimer = CurrentTime;
        My_PushToSolenoidStack(SO_POP_CENTER, 5, true);
      }
      return;
    case 4: 
      if (CurrentTime - BallSearchTimer > 250) {
        BallSearchStatus += 1;
        BallSearchTimer = CurrentTime;
        My_PushToSolenoidStack(SO_DTARGET_RESET, 5, true);
      }
      return;
    case 5: 
      if (CurrentTime - BallSearchTimer > 250) {
        BallSearchStatus += 1;
        BallSearchTimer = CurrentTime;
        My_PushToSolenoidStack(SO_SLING_LEFT, 5, true);
      }
      return;
    case 6: 
      if (CurrentTime - BallSearchTimer > 250) {
        BallSearchStatus += 1;
        BallSearchTimer = CurrentTime;
        My_PushToSolenoidStack(SO_SLING_RIGHT, 5, true);
      }
      return;
    case 7: 
      if (CurrentTime - BallSearchTimer > 250) {
        BallSearchStatus += 1;
        BallSearchTimer = CurrentTime;
        My_PushToSolenoidStack(SO_OUTHOLE, 5, true);
      }
      return;
    case 8: 
      if (CurrentTime - BallSearchTimer > 250) {   // Search concluded, no playfield switch hit. Ignore tilt if selected.
        BallSearchStatus += 1;
        BallSearchTimer = CurrentTime;
        if (BALL_SEARCH_TILT) {
          TiltIgnore = 1;
          RPU_SetLampState(LA_TILT, 1, 0, 250);
          PlaySound(VoiceTiltOff, 500); // We may be here for a very long time, doctor
        }
      }
      return;
    case 9:                                       // Still no switch hit. No further action taken.  
      return;
  }
}
...
int InitNewBall(bool curStateChanged, int ballNum) {
  ...
  if (curStateChanged) {
    FirstBall = (ballNum == 1 && !PlayerShootsAgain && ScoreAwardStates[CurrentPlayer] == 0);

    RPU_ClearSwitches();  // Added to clear solenoids and switches, ensure no carryover from previous game/player/ball.
    // RPU_TurnOffAllLamps();
    for (i = 0; i < RPU_MAX_LAMPS; ++i) {
      if (i != LA_SAME_PLAYER_SHOOTS_AGAIN && i != LA_SHOOT_AGAIN) RPU_SetLampState(i, 0, 0, 0); // Turn off all lights, but do not change SPSA
    }
    if (Credits > 0) RPU_SetLampState(LA_CREDIT_INDICATOR, 1);  // Credit light stays on
    SetUpNacelles(NACELLE_PATTERN);
    ...
    PhaserBonus = 1;
    scoreChimes[0] = scoreChimes[1] = scoreChimes[2] = 0;
    ...
    My_PushToSolenoidStack(SO_DTARGET_RESET, 15);
    My_PushToTimedSolenoidStack(SO_OUTHOLE, 5, BallSaveStartTime = CurrentTime + 250);
  }

  if (!RPU_ReadSingleSwitchState(SW_OUTHOLE)) {  // Proceed once ball reaches outhole
    BallTimeInTrough = 0;

    // Determine game to be played with current ball. Handle odd case where GameMode has been set to zero
    switch (GameMode != 0 ? GameMode : holdGameMode) {
      case 1: // Regular Gameplay
      case 2: // Kids' Mode
        if (ScoreAwardStates[CurrentPlayer] >= 1 && MiniGamePlayed[CurrentPlayer] == 0) {
          MiniGamePlayed[CurrentPlayer] = 1;
          return gamePlayed = MACHINE_STATE_MINI_GAME_1;
        } else if (ScoreAwardStates[CurrentPlayer] >= 2 && MiniGamePlayed[CurrentPlayer] == 1) {
          MiniGamePlayed[CurrentPlayer] = 2;
          return gamePlayed = MACHINE_STATE_MINI_GAME_2;
        } else if (ScoreAwardStates[CurrentPlayer] >= 3 && MiniGamePlayed[CurrentPlayer] == 2) {
          MiniGamePlayed[CurrentPlayer] = 3;
          return gamePlayed = MACHINE_STATE_MINI_GAME_3;
        } else
          return gamePlayed = MACHINE_STATE_SKILL_SHOT;
        break;
...
int RunSelfTest(int curState, boolean curStateChanged) {
  ...
  if (curStateChanged) {
    EELightShow = WinnerLightShow = PhotonLightShow = PhaserLightShow = MatchLightShow = AttractLightShow = false;
    CurrentNumPlayers = 0;
    CurrentPlayer = 0;
    CurrentScores[0] = 0;
  ...
  if (EELightShow) EnterpriseExplodes();
  if (WinnerLightShow) WinnerLights();
  if (MatchLightShow) MatchMode(0);
  if (AttractLightShow) RunAttractMode(MACHINE_STATE_TEST_LAMPS, 0);
  scoreChimes[0] = scoreChimes[1] = scoreChimes[2] = 0;

  if (CurrentTime > MiniGamePauseTimer) EELightShow = WinnerLightShow = false;
  ...
  if (curState >= MACHINE_STATE_TEST_DONE) {
    ...
    if (returnState >= 20000) {  // Fudged to play light shows in the main program!
      SelfTestLightShows(returnState - 20000);
      returnState = MACHINE_STATE_TEST_LAMPS;
    } else if (returnState >= 10000) {  // Fudged to play sounds in the main program!
      StopAudio();
      PlaySound(returnState == 10005 ? 90 : returnState - 10000);  // Play sound identified in sound test
      returnState = curState; // return state restored to original value
    }
...
int RunAttractMode(int curState, boolean curStateChanged) {
  ...
  if (curState == MACHINE_STATE_TEST_LAMPS) return returnState;  // self-test lamp test ends here!
  ...
  if ((CurrentTime / 6500) % 2 == 1) {  // Alternate displays between high score and latest player scores (reversed order, player scores first)
    ...
    AttractHeadMode = 1;
    SetDisplayAttract(HighScore[GameMode-1], HighScore[GameMode-1], HighScore[GameMode-1], HighScore[GameMode-1], CurrentTime - AttractStartAnimation);
  } else {
  ...
  if (LongPressAttract()) returnState = MACHINE_STATE_INIT_GAMEPLAY;
    ...
    if (!BadSwitches())
      switch (switchHit) {
        case SW_CREDIT_BUTTON:
          if (AddPlayer(true)) {
            holdGameMode = GameMode;
            GameMode = 0;
            LongPressTimer = CurrentTime + 1000;
          }
          break;
        ...
        case SW_SELF_TEST_SWITCH:
          if ((CurrentTime - GetLastSelfTestChangedTime()) > 5000) {  // Needs l o n g delay for some reason
            returnState = MACHINE_STATE_TEST_LAMPS;
            SetLastSelfTestChangedTime(CurrentTime);
          }
          break;
        ...
        case SW_SAUCER:
          if (CurrentTime > SaucerHitTime + 750) {  // If ball sitting in saucer, kick it out
        ...
        default:  // Unknown switch! Error? Clear all switches.
          RPU_ClearSwitches();
          // PlaySound(SFXError[2]); // Error message played. The old game's Star Trek theme.
          // SaucerHitTime = CurrentTime; // Prevents saucer from popping below
          break;
      }
  }
...
void SetDisplayAttract(unsigned long score1, unsigned long score2, unsigned long score3, unsigned long score4, unsigned long time) {
  ...
  for (byte i = 0; i < 4; ++i) {
    if (scores[i] == 0) {
      ...
      else if (Quarter >= 40 && Quarter < 48) RPU_SetDisplay(i, scores[i] / divisor[47 - Quarter], true, 0);                                                             // scroll right
      else if (Quarter == 16 || Quarter == 17 || Quarter == 20 || Quarter == 21 || Quarter == 24 || Quarter == 25 || Quarter == 28 || Quarter == 29 || Quarter >= 48) {  // Flash off
        RPU_SetDisplay(i, 0);
        RPU_SetDisplayBlank(i, 0);
      } else
...
// #################### LIGHTSHOW FOR ENTERPRISE EXPLODING ####################
void EnterpriseExplodes() {
  unsigned long timer = CurrentTime - (MiniGamePauseTimer - 100 * (unsigned long)SoundTimings[SFXEnterpriseExplodes][1]);
  if (timer <= 1100) {  // Lights explode outward
    for (i = 0; i < 60; ++i)
      if (timer * 254 / 1100 >= LampPlacement[i].distance)  // 250 should be used here and below, except an odd timing issue required me to use 254.
        RPU_SetLampState(i, 1);
  } else if (timer <= 2200) {  // Lights off outward
    for (i = 0; i < 60; ++i)
      if ((timer - 1100) * 254 / 1100 >= LampPlacement[i].distance)
        RPU_SetLampState(i, 0);
  } else if (timer <= 2800) {  // Secondary explosion outward, including backglass
    for (i = 0; i < 60; ++i)
      if ((timer - 2200) * 254 / 600 >= LampPlacement[i].distance || LampPlacement[i].distance == 255)
        RPU_SetLampState(i, 1);
  } else if (timer <= 3000) {  // Remain 100% lit
    // wait...
  } else if (timer <= 4200) {  // lights off inward
    for (i = 0; i < 60; ++i)
      if (254 - (timer - 3000) * 254 / 1200 <= LampPlacement[i].distance || LampPlacement[i].distance == 255)
        RPU_SetLampState(i, 0);
  }
}
...
// #################### LIGHTSHOW FOR MINI GAME WINNER ####################
void WinnerLights() {
  unsigned long timer = CurrentTime - (MiniGamePauseTimer - 100 * SoundTimings[SFXSkillShot][1]);
  if (timer > 3500) timer -= 3500;  // To be executed twice in 7 seconds
  if (timer < 1750) {               // Lights up
    timer = timer * 300 / 1750;
    for (i = 0; i < 60; ++i) {
      RPU_SetLampState(i, 0);
      if (LampPlacement[i].vertical <= timer && LampPlacement[i].vertical >= timer - 50)
        RPU_SetLampState(i, 1);
    }
  } else {  // Lights down
    timer = (timer - 1750) * 300 / 1750;
    for (i = 0; i < 60; ++i) {
      RPU_SetLampState(i, 0);
      if (LampPlacement[i].vertical <= 300 - timer && LampPlacement[i].vertical >= 250 - timer)
        RPU_SetLampState(i, 1);
    }
  }
}
...
void FirePhotonTorpedoes() {
  if (PhotonLightShow && CurrentTime > PhotonLightShowTimer2) {  // Handle light show
    // if (PhotonScore != 0) RPU_SetDisplayFlashCredits(CurrentTime, 400);
    switch (PhotonStep2) {
      case 0:
      ...
      case 38:
        ...
        RPU_SetLampState(BALLYMiddle[0], ScoreBALLY[CurrentPlayer][0]);
        RPU_SetLampState(BALLYMiddle[1], ScoreBALLY[CurrentPlayer][1]);
        RPU_SetLampState(BALLYMiddle[2], ScoreBALLY[CurrentPlayer][2]);
        RPU_SetLampState(BALLYMiddle[3], ScoreBALLY[CurrentPlayer][3]);
        RPU_SetLampState(BALLYMiddle[4], ScoreBALLY[CurrentPlayer][4]);
  ...
  if (PhotonLightShow && CurrentTime > PhotonLightShowTimer) {  // Handle scoring and indicator lights
    switch (PhotonStep) {
      ...
      case 2:
      case 0:
        AddToScore(PhotonScore, true, 10);
        if (PhotonScore != 0) RPU_SetDisplayCredits(PhotonScore / 200);
        PhotonStep += 1;
      ...
      case 3:
      case 1:
        if (PhotonScore != 0) RPU_SetDisplayCredits(PhotonScore / 200, false);
        PhotonStep += 1;
        PhotonLightShowTimer += 400;
        break;
    }
...
void FireAllPhasers() {
  if (PhaserLightShow) RPU_SetDisplayFlashCredits(CurrentTime, 200);
...
int NormalGamePlay(boolean curStateChanged) {
  ...
  if (curStateChanged) {
    ...
    if (PhotonLightShow == true) {  // Timing issues on switch from skill shot to normal game play
      SaucerHitTime = CurrentTime;
      PhotonLightShowTimer = PhotonLightShowTimer2 = CurrentTime + 100 * SoundTimings[SFXSkillShot][1] + 50;
    }
  }
  ...
  // Music stops during skillshot bonus, to play fanfare.
  if (resumeMusic < CurrentTime) {
    resumeMusic = 0xFFFF;
    ResetGain(BackgroundMusic[1 + backgroundSound], bgGain);
    // scoreChimes[0] = scoreChimes[0] = scoreChimes[0] = 0;
    // CountingDisplay(CurrentPlayer, DisplayValue = CurrentScores[CurrentPlayer], true, 2, true);
  }
...
int BallOver(boolean curStateChanged) {
  ...
  if (CurrentScores[CurrentPlayer] == InitialScore && NumTiltWarnings < MAX_TILT_WARNINGS && !PFValidated)  // Ball has fallen back into outhole
    return MACHINE_STATE_INIT_NEW_BALL;

  // if we have remaining ball saves, and we're under the time limit, then save the ball
  if ((BallSaveUsed[CurrentPlayer] > 0) && (CurrentTime < (BallSaveStartTime + ballSaveNumMS)) && !PlayerShootsAgain && gamePlayed == MACHINE_STATE_SKILL_SHOT) {
  ...
  else if ((BallSaveUsed[CurrentPlayer] > 0) && (CurrentScores[CurrentPlayer] < InitialScore + ballSaveScore) && !PlayerShootsAgain && gamePlayed == MACHINE_STATE_SKILL_SHOT) {
    BallSaveUsed[CurrentPlayer]--;
    RPU_SetLampState(LA_SAME_PLAYER_SHOOTS_AGAIN, 1);
    RPU_SetLampState(LA_SHOOT_AGAIN, 1);
    PlayerShootsAgain = true;
...
// Check if minigame to be played with next ball. Go to InitNewBall without setting PlayerShootsAgain.
  ...
  if (ScoreAwardStates[CurrentPlayer] >= 2 && MiniGamePlayed[CurrentPlayer] == 1) {
    RPU_SetLampState(LA_SAME_PLAYER_SHOOTS_AGAIN, 1);
    RPU_SetLampState(LA_SHOOT_AGAIN, 1);
    return MACHINE_STATE_INIT_NEW_BALL;
  }

  if (ScoreAwardStates[CurrentPlayer] >= 3 && MiniGamePlayed[CurrentPlayer] == 2) {
    RPU_SetLampState(LA_SAME_PLAYER_SHOOTS_AGAIN, 1);
    RPU_SetLampState(LA_SHOOT_AGAIN, 1);
    return MACHINE_STATE_INIT_NEW_BALL;
  }
...
int MatchMode(boolean curStateChanged) {
  ...
  if (curStateChanged) {
    ...
    if (highestScore > HighScore[GameMode - 1]) {
      HighScore[GameMode - 1] = highestScore;
      ...
      RPU_WriteULToEEProm(GameMode == 1 ? RPU_HIGHSCORE_EEPROM_START_BYTE : RPU_HIGHSCORE_EEPROM_START_BYTE_2 + (GameMode - 2) * 20, HighScore[GameMode - 1]);
      for (byte count = 0; count < CurrentNumPlayers; count++) {
        RPU_SetDisplay(count, CurrentScores[count], true, 2);
      }
      DisplayValue = CurrentScores[CurrentPlayer];
  ...
  if (dipMatch) {
    ...
    StopAudio();
    if (PersonalGoalOn) {
      if (highestScore < PersonalGoal[GameMode - 1])
...
int ShowMatchSequence(boolean curStateChanged) {
  ...
  // *** TEST!! Set match digit and score digits to zero

  // CurrentScores[0] -= CurrentScores[0] % 100; // set endvals to 00
  // CurrentScores[1] -= CurrentScores[1] % 100; // set endvals to 00
  // CurrentScores[2] -= CurrentScores[2] % 100; // set endvals to 00
  // CurrentScores[3] -= CurrentScores[3] % 100; // set endvals to 00
  // MatchDigit = 0; // set match to zero

  // *** Random flashing light display
  ...
  if (CurrentTime > (MatchSequenceStartTime + MatchDelay)) {
    ...
    // *** Rotate values through credit / match display
    ...
    if (NumMatchSpins < 15) {
      ...
      if (NumMatchSpins < 10)
        ...
        workingMatch[3] = finalMatch[NumMatchSpins - 10];
    }

    // *** Determine matches

    if (NumMatchSpins >= 15 && NumMatchSpins < 19) {
      if ((CurrentScores[NumMatchSpins - 15] % 100) == MatchDigit && (NumMatchSpins - 15 < CurrentNumPlayers)) {
        ScoreMatches |= (1 << (NumMatchSpins - 15));
        AddCredit(0, CreditsperCoin[0], CoinsperCredit[0], 1);
        TotalReplays += 1;
        RPU_WriteULToEEProm(RPU_TOTAL_REPLAYS_EEPROM_START_BYTE, TotalReplays);
        My_PushToSolenoidStack(SO_KNOCKER, 3, true);
        RPU_SetLampState(LA_MATCH, 1);
        MatchDelay = 300;
      } else
        MatchDelay = 0;
    ...
    if (NumMatchSpins == 19) {
      if (PersonalGoalOn) {
        ...
        if (highestScore < PersonalGoal[GameMode - 1])
          PlaySound(VoiceLose[random(1, 1 + VoiceLose[0])], 600);
        else
          PlaySound(VoiceWin[random(1, 1 + VoiceWin[0])], 600);
      }
      MatchDelay = (ScoreMatches == 0 ? 300 : 4000);
    }
    ...
    if (NumMatchSpins == 20) {
      ...
      AttractCalloutTimer = CurrentTime - ATTRACT_SPEECH_TIMER;
      AttractWait = 10000;
      NumMatchSpins = 99;  // Stops function from continuing, when run in self-test
  ...
  for (byte count = 0; count < 4; count++) {
    if ((ScoreMatches >> count) & 0x01) {  // If this score matches, we're going to flash the last two digits
      if ((CurrentTime / 250) % 2)
        RPU_SetDisplayBlank(count, RPU_GetDisplayBlank(count) & 0x0F);  // 0x0F 001111
      else
        RPU_SetDisplayBlank(count, RPU_GetDisplayBlank(count) | 0x30);  // 0x30 110000
    }
  }
...
int RunGamePlayMode(int curState, boolean curStateChanged) {
  ...
  } else if (curState == MACHINE_STATE_MINI_GAME_3) {
    returnState = MiniGame3(curStateChanged);
  ...
  BallSearchRoutine(); // Determine if ball search required and act as necessary
  ...
  if (NumTiltWarnings < MAX_TILT_WARNINGS) {
    while ((switchHit = RPU_PullFirstFromSwitchStack()) != SWITCH_STACK_EMPTY) {
      ...
      if (CheckforPlayfieldSwitch(switchHit)) break;  // part of ball search routine. Ignore certain switches until tiltignore turned off.
      ...
      if (!BadSwitches())
        switch (switchHit) {
          ...
          case SW_SHOOTER_LANE:
            ...
            if (!PFValidated && !PlayerShootsAgain && (curState == MACHINE_STATE_MINI_GAME_1 || curState == MACHINE_STATE_MINI_GAME_2 || curState == MACHINE_STATE_MINI_GAME_3)) {
              RPU_SetLampState(LA_SAME_PLAYER_SHOOTS_AGAIN, 0);
              RPU_SetLampState(LA_SHOOT_AGAIN, 0);
            }
            if (PFValidated && !(curState == MACHINE_STATE_MINI_GAME_1 || curState == MACHINE_STATE_MINI_GAME_2 || curState == MACHINE_STATE_MINI_GAME_3)) {
              PFValidated = false;
              SetUpPhasers();
            }
            if (curState == MACHINE_STATE_MINI_GAME_3) 
              hiding = true;
            
            break;

          case SW_CREDIT_BUTTON:
            if (CurrentPlayer == 0 && CurrentScores[CurrentPlayer] == 0 && GameMode != 0 && CurrentBallInPlay < 2) { // Check for long press to set game mode
              holdGameMode = GameMode;
              GameMode = 0;
              LongPressTimer = CurrentTime + 1000;
              break;
          ...
          case SW_DROP_TARGET_A_HIGH:
            ...
            if (curState == MACHINE_STATE_MINI_GAME_3 && !MiniGame2Score[switchHit]) {
              MiniGame2Score[switchHit] = true;
              AddToScore(10000, true);
              PlaySound(VoiceKobayashiMaru[1 + MiniGame2Score[0] + MiniGame2Score[1] + MiniGame2Score[2] + MiniGame2Score[3]]);  // Messages played in order as targets dropped
              if (MiniGame2Score[0]) RPU_SetLampState(LA_2X, 0);
              if (MiniGame2Score[1] && MiniGame2Score[2]) RPU_SetLampState(LA_SPEC_3X, 0);
              if (MiniGame2Score[3]) RPU_SetLampState(LA_3X_2X, 0);
              break;
          ...
          case SW_SAUCER:
            ...
            if (SaucerHitTime == 0 || (CurrentTime - SaucerHitTime) > 500) {
              ...
              if (curState == MACHINE_STATE_MINI_GAME_1 || curState == MACHINE_STATE_MINI_GAME_2 || curState == MACHINE_STATE_MINI_GAME_3) {
                BallInSaucer = true;
                break;
              }
              ...
              if ((!BALLYScored(SkillShotMade ? 100 * SoundTimings[SFXSkillShot][1] : 0)) && (!SkillShotMade))
                PlaySound(VoiceSaucer, 500);  // Arm photon torpedos

              if (SkillShotMade) {  // Allow fanfare to play a while before anything else
                ResetGain(BackgroundMusic[1 + backgroundSound], -70);
                DisplayTimer = resumeMusic = CurrentTime + 100 * SoundTimings[SFXSkillShot][1];  // Resume background music after fanfare
                My_PushToTimedSolenoidStack(SO_SAUCER, 5, CurrentTime + 100 * SoundTimings[SFXSkillShot][1], true);
                BallInSaucer = false;  // We don't want other code to activate the saucer solenoid
                break;                 // Avoid "BallInSaucer = true" below
              }
              BallInSaucer = true;
            }
            break;
          ...
          case SW_HYPER_SPACE_UPPER:
            ...
            } else if (dipThresholdAward == 2) {  // Award 2: Extra ball + 5000, or 25000 + 5000 if same player shoots again. 10000 second and subsequent times.
              if (PlayerShootsAgain) {
          ...
          case SW_500_POINT_TARGET:
            ...
            if (curState == MACHINE_STATE_MINI_GAME_3) {
              if (10 >= ++KlingonCount)
                AddToScore(5000, true);
              CountdownTimer = CurrentTime + 1000 * (FinalTime = KOBAYASHI_MARU_TIME);
              PlaySound(SFXExplosion4, 0, voiceGain + 2);  // Explosion
              break;
            }
            if (curState == MACHINE_STATE_MINI_GAME_1)
          ...
          case SW_10_POINT_REBOUND:
            ...
            // Hiding / Running indicator not reset by 10 point rebound.
            if (curState == MACHINE_STATE_MINI_GAME_1 || curState == MACHINE_STATE_MINI_GAME_2 || curState == MACHINE_STATE_MINI_GAME_3)
              break;
          ...
          case SW_KLINGON_A:
            ...
            if (curState == MACHINE_STATE_SKILL_SHOT) {
              ...
              if (ShootFor == 1) {
                ...
                if (SkillShotDoubler > 1) {
                  ...
                  DisplayValue = CurrentScores[CurrentPlayer] += 300 * SkillShotDoubler;  // Note: Points awarded immediately
                  RPU_SetDisplay(CurrentPlayer, 300 * SkillShotDoubler, true, 2);
                } else
                  AddToScore(300);
              }
              if (SkillShotMade) {  // Allow fanfare to play a while before anything else
                ResetGain(BackgroundMusic[1 + backgroundSound], -70);
                DisplayTimer = CurrentTime + 2000;
                resumeMusic = CurrentTime + 100 * SoundTimings[SFXSkillShot][1];  // Resume background music after fanfare
              }
            } else
              AddToScore(300);
            ...
            if (++SaucerBALLY[CurrentPlayer] > 4) SaucerBALLY[CurrentPlayer] = 0;  // Move saucer to next letter
            RPU_SetLampState(LA_SAUCER_B + SaucerBALLY[CurrentPlayer], 1);         // Turn on letter in saucer lights
            BALLYScored(SkillShotMade ? 100 * SoundTimings[SFXSkillShot][1] : 0);  // Check if BALLY scored, set up photon display
            break;
          ...
          case SW_LEFT_SLINGSHOT:
            PFValidated = true;
            hiding = false;
            PlaySound(SFXSlingshot, 100);  // Phaser
            if (curState == MACHINE_STATE_MINI_GAME_1 || curState == MACHINE_STATE_MINI_GAME_2 || curState == MACHINE_STATE_MINI_GAME_3)
              break;
  ...
  // Handle score award thresholds. Star Trek has three challenge levels instead of award levels.

  if (CurrentScores[CurrentPlayer] >= ScoreAward1[GameMode-1] && ScoreAward1[GameMode-1] > 0 && ScoreAwardStates[CurrentPlayer] == 0) {  // Mini-Game 1
    ScoreAwardStates[CurrentPlayer] = 1;
    RPU_SetLampState(LA_SAME_PLAYER_SHOOTS_AGAIN, 1);
    RPU_SetLampState(LA_SHOOT_AGAIN, 1);
  }
  if (CurrentScores[CurrentPlayer] >= ScoreAward2[GameMode-1] && ScoreAward2[GameMode-1] > 0 && ScoreAwardStates[CurrentPlayer] == 1) {  // Mini-Game 2
    ScoreAwardStates[CurrentPlayer] = 2;
    RPU_SetLampState(LA_SAME_PLAYER_SHOOTS_AGAIN, 1);
    RPU_SetLampState(LA_SHOOT_AGAIN, 1);
  }
  if (CurrentScores[CurrentPlayer] >= ScoreAward3[GameMode-1] && ScoreAward3[GameMode-1] > 0 && ScoreAwardStates[CurrentPlayer] == 2) {  // Mini-Game 3
...
void SetupGameMode() {
  kidMode = 0;
  if (GameMode == 1) {
    ReadAllEEProm();  // Get settings from memory
...
// #################### LONG PRESS ####################
boolean LongPress() { // Long press of game button to set game mode 
  if (LongPressTimer != 0xFFFFFFFF) {
    if (LongPressTimer > CurrentTime) {
      if (!RPU_ReadSingleSwitchState(SW_CREDIT_BUTTON)) { // Game button released
        LongPressTimer = 0xFFFFFFFF;
        if (GameMode == 0) {
          AddPlayer();
          GameMode = holdGameMode;
          return false;
        } else {
          SetupGameMode();
          return (GameMode != holdGameMode);
        }
      } 
    } else { // Timer passed
        if (++GameMode > 3) GameMode = 1;
        LongPressTimer = CurrentTime + 1000;
        RPU_SetDisplayCredits(GameMode);
        PlaySound(SFXChime10);
        return false;
    }
  }
  return false;
...
// #################### LONG PRESS ATTRACT ####################
boolean LongPressAttract() { // Long press of game button while in attract mode
  if (LongPressTimer != 0xFFFFFFFF) {
    if (LongPressTimer > CurrentTime) {
      if (!RPU_ReadSingleSwitchState(SW_CREDIT_BUTTON)) { // Game button released
        LongPressTimer = 0xFFFFFFFF;
...
int SkillShot(boolean curStateChanged) {
  ...
  if (curStateChanged) {
    ...
    if (LongPressTimer == 0xFFFFFFFF) RPU_SetDisplayCredits(Credits, dipCreditsDisplayed);

    RPU_SetLampState(BALLYMiddle[ShootFor], 1, 0, 250); // Set all flashing lights for skill shot
  ...
  // Long press to set game mode
  if (LongPress()) return MACHINE_STATE_INIT_NEW_BALL;
  ...
  if (!PFValidated) {
    ...
    RPU_SetLampState(LA_B_LANE, !ScoreBALLY[CurrentPlayer][0]);  // Stop all flashing lights
    RPU_SetLampState(LA_A_LANE, !ScoreBALLY[CurrentPlayer][1]);
    if (!PhotonLightShow) RPU_SetLampState(BALLYMiddle[ShootFor], ScoreBALLY[CurrentPlayer][ShootFor]);
    for (i = 0; i < 5; ++i)
    ...
    if (BallTimeInTrough == 0) {
      ...
      if ((CurrentTime - BallTimeInTrough) > 500) {
        ...
        if (CurrentScores[CurrentPlayer] == InitialScore) {
          My_PushToTimedSolenoidStack(SO_OUTHOLE, 5, CurrentTime + 100);
...
int MiniGame1(boolean curStateChanged) {
  ...
  // Check to see if ball is in the outhole.
  if (RPU_ReadSingleSwitchState(SW_OUTHOLE)) {
    if (BallTimeInTrough != 0 && (CurrentTime - BallTimeInTrough) > 500) // Avoids outhole set off by other than ball in hole, e.g. vibration!
      if (MiniGameStarted && !MiniGameEnded) {                           // (It happened to me!)
        MiniGameEnded = true;
        SetUpEnterpriseExplodes();
      }
...
int MiniGame2(boolean curStateChanged) {
  ...
  if (curStateChanged) {
    ...
    RPU_SetDisplayCredits(60, true);
    ShowCountdownOnLadder(60);
  ...
  CountingDisplay(CurrentPlayer, CurrentScores[CurrentPlayer], true, 2);
  if (FinalTime != 255) RPU_SetDisplayFlashCredits(CurrentTime, 250);
  if (EELightShow) EnterpriseExplodes();
  if (WinnerLightShow) WinnerLights();

  if (CurrentTime - CountdownTimer > 1700 && !MiniGameEnded) {  // Start Timer when auto-destruct voice ends, 1700ms
    DestructTimer = (1000 + 1700 + 60000 - (CurrentTime - CountdownTimer)) / 1000;
    RPU_SetDisplayCredits(DestructTimer, true, true);  // +1000 to round up
    ShowCountdownOnLadder(DestructTimer);
  }

  if (CurrentTime - CountdownTimer > 61700 && !MiniGameEnded) {  // Time is up!
    // MiniGame2Score[0] = MiniGame2Score[1] = MiniGame2Score[2] = MiniGame2Score[3] = false;
    RPU_SetDisableFlippers(true);
    FinalTime = DestructTimer;
...
// #################### MINI GAME 3: Kobayashi Maru ####################
int MiniGame3(boolean curStateChanged) {
  int returnState = MACHINE_STATE_MINI_GAME_3;

  if (curStateChanged) {
    PlaySound(VoiceKobayashiMaru[1], 500);     // This is the Kobayashi Maru...
    if (backgroundOn) {
      StopBackgroundSong();
      PlaySound(BGMiniGame3Hide, 100 * SoundTimings[VoiceKobayashiMaru[1]][1] - 1000);  // Background music for hidden ship
    }

    CountdownTimer = 0xFFFFFFFF; // Ship is hidden, countdown stopped
    ...
    RPU_SetLampState(LA_3X_2X, 1, 0, 250);
    RPU_SetLampState(LA_SPEC_3X, 1, 0, 250);
    RPU_SetLampState(LA_KLINGON_L1, 1, 1);
    RPU_SetLampState(LA_KLINGON_L2, 1, 1);
    RPU_SetLampState(LA_KLINGON_Y, 1, 1);
    RPU_SetLampState(LA_300_OR_3000_TARGET, 1, 1);
    RPU_SetLampState(LA_HYPER_SPACE_10K, 1, 1);

    RPU_SetDisplayCredits(KOBAYASHI_MARU_TIME, false);
    ShowCountdownOnLadder(0);
  ...
  // HANDLE SITUATION WHERE SHIP GOES FROM RUNNING TO HIDING (OR VICE VERSA)
  if (hiding != holdhiding && !MiniGameEnded) {
    holdhiding = hiding;
    if (backgroundOn)
      if (hiding)
        PlayBackgroundSong(BGMiniGame3Hide); // Hiding music
    else
        PlayBackgroundSong(BGMiniGame3Run);  // Running music

    if (hiding) {                            // Ship is hidden, countdown stopped
      FinalTime = 255;
      CountdownTimer = 0xFFFFFFFF;
      RPU_SetDisplayCredits(KOBAYASHI_MARU_TIME, false);
      ShowCountdownOnLadder(0);
    } else {                                 // Ship is running, countdown begins
      CountdownTimer = CurrentTime + 1000 * (FinalTime = KOBAYASHI_MARU_TIME);
      RPU_SetDisplayCredits(KOBAYASHI_MARU_TIME, true);
      ShowCountdownOnLadder(KOBAYASHI_MARU_TIME);
    }
  }
  ...
  CountingDisplay(CurrentPlayer, CurrentScores[CurrentPlayer], true, 2);
  if (MiniGameEnded && !hiding) RPU_SetDisplayFlashCredits(CurrentTime, 250);
  if (EELightShow) EnterpriseExplodes();
  if (WinnerLightShow) WinnerLights();

  if (CurrentTime < CountdownTimer && !MiniGameEnded && !hiding) {
    DestructTimer = (999 + (CountdownTimer - CurrentTime)) / 1000;
    RPU_SetDisplayCredits(DestructTimer, true, true);  // +999 to round up
    ShowCountdownOnLadder(DestructTimer);
  }

  if (CountdownTimer - CurrentTime <= 5000 && CurrentTime - WarningTime > 5500 && !MiniGameEnded) {
    PlaySound(SFXChime10); // 5 second warning - 10 chime
    WarningTime = CurrentTime;
  }

  if (CurrentTime >= CountdownTimer && !MiniGameEnded) {  // Time is up!
    RPU_SetDisableFlippers(true);
    FinalTime = 0;
    SetUpEnterpriseExplodes();
    MiniGameEnded = true;
  ...
  if (MiniGame2Score[0] && MiniGame2Score[1] && MiniGame2Score[2] && MiniGame2Score[3] && !dropsDown && !MiniGameEnded) {  // Found Kobayashi Maru. Now must escape!
    FinalTime = 0;
    AddToScore(10000, true, 10);
    dropsDown = true;
  }
...
// #################### MINIGAME AWARDS ####################
// ######### DETERMINE AWARD ON WINNING A MINIGAME #########
void MiniGameAwards() {
  
  if (dipThresholdAward == 1) // No award
    return;
  else if (dipThresholdAward == 2) { // Extra ball
    if (PlayerShootsAgain)
      AddToScore(25000, true, 10);
    else {
      PlayerShootsAgain = true;
      FreeBallCollected[CurrentPlayer] = true;
      RPU_SetLampState(LA_SAME_PLAYER_SHOOTS_AGAIN, 1);
  ...
  } else if (dipThresholdAward == 3 && !FreeGameCollected[CurrentPlayer]) { // Free game
    AddCredit(0, CreditsperCoin[0], CoinsperCredit[0]);
    My_PushToSolenoidStack(SO_KNOCKER, 3);
    FreeGameCollected[CurrentPlayer] = true;
    TotalReplays += 1;
...
boolean CheckIfDTargets4Down() {
  return (RPU_ReadSingleSwitchState(SW_DROP_TARGET_D_LOW) && RPU_ReadSingleSwitchState(SW_DROP_TARGET_C_MID_LOW) && RPU_ReadSingleSwitchState(SW_DROP_TARGET_B_MID_HIGH) && RPU_ReadSingleSwitchState(SW_DROP_TARGET_A_HIGH));
}
...
// ########## SELF TEST CAN RUN ANY OF THE LIGHTSHOWS ##########
void SelfTestLightShows(int LightShow) {
  EELightShow = WinnerLightShow = PhotonLightShow = PhaserLightShow = MatchLightShow = AttractLightShow = false;
  PhaserBonus = 0;
  if (LightShow >= 100) {
    if (SetUpNacelles(LightShow - 100))
      RPU_SetDisplay(0, LightShow - 100, true);
    else {
      RPU_PushToSwitchStack(SW_CREDIT_BUTTON); // double-click to end nacelles
      RPU_PushToSwitchStack(SW_CREDIT_BUTTON);
    }
    return;
  }
  for (i = (LightShow == 0); i < 4; ++i) {  // Blank all displays, except display 1 during test zero
    RPU_SetDisplay(i, 0);
    RPU_SetDisplayBlank(i, 0x00);
  }

  switch (LightShow) {
    case 1:  // Setup Photon Torpedoes
      StopBackgroundSong();
      BALLYScored(0, true);
      break;
    case 2:  // Setup Phasers
      StopBackgroundSong();
      SetUpPhasers();
      break;
    case 3:  // Setup Enterprise Explosion
      SetUpEnterpriseExplodes();
      break;
    case 4:  // Setup Winner Light Show
      SetUpWinner();
      break;
    case 5:  // Setup Match Light Show
      SetUpMatch();
      break;
    case 6:  // Setup Attract Light Show
      SetUpAttract();
      break;
  }
}
...
boolean BALLYScored(unsigned long delayPT = 0, boolean selfTest = 0) {
  ...
  if (scored) {
    PlaySound(VoicePhotonTorpedoesFire, delayPT + 450);  // Photon torpedoes, fire! 450 covers "chime" sound
    PhotonLightShow = true;
    PhotonStep = PhotonStep2 = 0;
    PhotonLightShowTimer = PhotonLightShowTimer2 = CurrentTime + delayPT + 250;
    for (i = 0; i < 5; ++i) {
      ScoreBALLY[CurrentPlayer][i] = 0;
      RPU_SetLampState(LA_B_LANE + i, 1);
    }
    if (PhotonBonus[CurrentPlayer] == 0) PhotonScore = 2000;
    else if (PhotonBonus[CurrentPlayer] == 1) PhotonScore = 5000;
    else if (PhotonBonus[CurrentPlayer] == 2) PhotonScore = 10000;
    else if (PhotonBonus[CurrentPlayer] == 3 && ((dipThresholdAward == 1) || (dipThresholdAward == 2 && PlayerShootsAgain))) PhotonScore = 10000;
    else if (PhotonBonus[CurrentPlayer] == 4 && dipThresholdAward == 1) PhotonScore = 10000;
    else if (PhotonBonus[CurrentPlayer] == 4 && (!dipBallySpecialAdjustment) && (dipThresholdAward == 2) && (FreeBallCollected[CurrentPlayer] || PlayerShootsAgain)) PhotonScore = 10000;
    else if (PhotonBonus[CurrentPlayer] == 4 && dipBallySpecialAdjustment && (dipThresholdAward == 2) && PlayerShootsAgain) PhotonScore = 10000;
    else PhotonScore = 0;
  }
  return scored;
}
...
void SetUpPhasers() {
  // StopAllButBG();
  PlaySound(VoiceFireAllPhasers[BonusXState[CurrentPlayer]], 0, bgGain);  // Fire all phasers + 1, 2, or 3 phaser sounds. Voice + SFX, compromise at BG.
  PhaserLightShow = true;
  PhaserStep = 0;
  PhaserLightShowTimer = 1000 + CurrentTime;
  RPU_SetDisplayCredits(PhaserBonus);
}


// ########## SET UP ENTERPRISE EXPLODES SOUND AND LIGHT SHOW ##########
void SetUpEnterpriseExplodes() {
  RPU_TurnOffAllLamps();
  StopBackgroundSong();
  LampPlacement[13].distance = AlternativeLamps[0].distance;
  LampPlacement[14].distance = AlternativeLamps[1].distance;
...
// ########## SET UP MINIGAME WINNER SOUND AND LIGHT SHOW ##########
void SetUpWinner() {
  RPU_TurnOffAllLamps();
  StopBackgroundSong();
  LampPlacement[13].vertical = AlternativeLamps[2].vertical;
  LampPlacement[13].horizontal = AlternativeLamps[2].horizontal;
  LampPlacement[14].vertical = AlternativeLamps[3].vertical;
  LampPlacement[14].horizontal = AlternativeLamps[3].horizontal;
  PlaySound(SFXSkillShot, 0, bgGain);  // Opening fanfare
  MiniGamePauseTimer = CurrentTime + 100 * SoundTimings[SFXSkillShot][1];
  WinnerLightShow = true;
}


// ########## SET UP MATCH SOUND AND LIGHT SHOW ##########
void SetUpMatch() {
  CurrentScores[0] = 0;
  RPU_TurnOffAllLamps();
  StopBackgroundSong();
...
// ########## SET UP NACELLE LIGHTS ##########
boolean SetUpNacelles(byte dispNum) {
  switch (dispNum) {
    case 0: // Off
    RPU_SetLampState(LA_LEFT_NACELLE, 0);
    RPU_SetLampState(LA_RIGHT_NACELLE, 0);
    break;

    case 1: // Bright
    case 2: // Dim
    case 3: // Flicker
    case 4: // Lots of flicker!
    RPU_SetLampState(LA_LEFT_NACELLE, 1, dispNum - 1);
    RPU_SetLampState(LA_RIGHT_NACELLE, 1, dispNum - 1);
    break;

    case 5: // Dim, flashing
    RPU_SetLampState(LA_LEFT_NACELLE, 1, 1, 250);
    RPU_SetLampState(LA_RIGHT_NACELLE, 1, 1, 250);
    break;

    case 6: // Ship in distress
    RPU_SetLampState(LA_LEFT_NACELLE, 1, 2);
    RPU_SetLampState(LA_RIGHT_NACELLE, 1, 3);
    break;

    default:
      if (dispNum > 6 && dispNum <= 31) { // All brightness combinations (5 x 5 = 25, full down to off), plus invalid if exceeding max.
        byte i = (dispNum - 7) / 5;
        byte j = (dispNum - 7) % 5;
        RPU_SetLampState(LA_LEFT_NACELLE, i != 4, i);
        RPU_SetLampState(LA_RIGHT_NACELLE, j != 4, j);
        break;
      }
      else return false;
  }
...
// ########## SET UP ATTRACT SOUND AND LIGHT SHOW ##########
void SetUpAttract() {
  RPU_TurnOffAllLamps();
    SetUpNacelles(NACELLE_PATTERN);
  StopBackgroundSong();
  RunAttractMode(MACHINE_STATE_TEST_LAMPS, 1);
  AttractLightShow = true;
}
...
// ########## SHOW PHASER BONUS ON LADDER ##########
void ShowCountdownOnLadder(byte time) {

  // Show only the last ten seconds of any countdown
  if (time > 10) time = 0;
    for (i = 0; i < 10; ++i)
      RPU_SetLampState(LA_BONUS_1K + i, time > i);
  return;

  // Code for time up to 60 seconds (never run)
  RPU_SetLampState(LA_MIDDLE_SPECIAL, time >= 60, 0, 0);
  RPU_SetLampState(LA_MIDDLE_50K, time >= 50, 0, 0);
  RPU_SetLampState(LA_MIDDLE_10K, time >= 40, 0, 0);
  RPU_SetLampState(LA_BONUS_20K, time >= 20, 0, 0);
  RPU_SetLampState(LA_BONUS_10K, time >= 30 || (time >= 10 && time < 20), 0, 0);
  byte time10 = time % 10 + 10 * (time == 10);
  if (time > 10)
    for (i = 0; i < 9; ++i)
      RPU_SetLampState(LA_BONUS_1K + i, time10 == i + 1);
  else
    for (i = 0; i < 9; ++i)
      RPU_SetLampState(LA_BONUS_1K + i, time10 >= i + 1);
}
...
int RunExtendedSelfTest(int curState, boolean curStateChanged, unsigned long CurrentTime, byte resetSwitch, byte otherSwitch, byte endSwitch) {
  ...
  if (BadSwitches()) curSwitch = SWITCH_STACK_EMPTY;
  ...
  if (!RPU_ReadSingleSwitchState(resetSwitch) && CurrentTime - ExtLastResetPress > 400 && ExtLastResetPress != 0) {
    curSwitch = resetSwitch;
    ExtLastResetPress = 0;
  }

  if (curSwitch == otherSwitch) {
    if ((CurrentTime - ExtLastOtherPress) < 400) {
      otherDoubleClick = true;
      curSwitch = SWITCH_STACK_EMPTY;
    }
    ExtLastOtherPress = CurrentTime;
  }

  if (ExtResetHold != 0 && !RPU_ReadSingleSwitchState(resetSwitch)) {
    ExtResetHold = 0;
    ExtNextSpeedyValueChange = 0;
  }

  if (ExtResetHold != 0 && (CurrentTime - ExtResetHold) > 500) {
    resetBeingHeld = true;
    ExtLastResetPress = 0;
    if (ExtNextSpeedyValueChange == 0) {
      ExtNextSpeedyValueChange = CurrentTime;
      ExtNumSpeedyChanges = 0;
    }
  }
...
void KidSettings() {  // Set all DIP switches and game settings for beginner mode
  ...
  // HighScore[GameMode-1] = 50000;
  // ScoreAward1[GameMode-1] = 25000;
  // ScoreAward2[GameMode-1] = 75000;
  // ScoreAward3[GameMode-1] = 150000;
  // PersonalGoal[GameMode-1] = 75000;
  ballSaves = 5;
  ballSaveScore = 7000;
  ballSaveNumMS = 20000;
  for (byte i = 0; i < 4; i++) {
...
// #################### RESET ALL VARIABLES FOR A BEGINNER GAME ####################
void ChallengeSettings() {  // Set all DIP switches and game settings for challenge mode

  // DIP switches set
  dipBallsPerGame = 3;
  dipThresholdAward = 1;

  // Game settings in memory set
  // HighScore[GameMode-1] = 350000;
  ScoreAward1[GameMode-1] = 0;
  ScoreAward2[GameMode-1] = 0;
  ScoreAward3[GameMode-1] = 0;
  // PersonalGoal[GameMode-1] = 200000;
  ballSaves = 0;
  ballSaveScore = 5000;
  ballSaveNumMS = 15000;
  for (byte i = 0; i < 4; i++) {
...
void DeleteQueue() {
  for (iWT = 0; iWT < SOUND_STACK_SIZE; ++iWT)
    SoundQueue[iWT].type = 0xFF;
}