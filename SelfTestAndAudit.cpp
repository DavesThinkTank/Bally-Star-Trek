...
int RunBaseSelfTest(int curState, boolean curStateChanged, unsigned long CurrentTime, byte resetSwitch, byte otherSwitch, byte endSwitch) {
  ...
  if (curState == MACHINE_STATE_TEST_LAMPS) {  //                                                            *** Test Lamps ***
    ...
    if (resetDoubleClick && LightShow >= 100) { // End nacelle patterns
      LightShow = 0;
      CurValue = 99;
      RPU_SetDisplay(0, CurValue, true);
      for (int count = 0; count < RPU_MAX_LAMPS; count++) {
    ...
    else if (resetDoubleClick && (CurValue == 11 || CurValue == 12) && LightShow == 0) { // Show nacelle patterns
      LightShow = 101;
      returnState = 20000 + LightShow;
    }
    else if (LightShow >= 100 && (curSwitch == resetSwitch || (resetBeingHeld && CurrentTime > LastSolTestTime + 250))) {
        LastSolTestTime = CurrentTime;
        LightShow += 1;
      returnState = 20000 + LightShow;
    }
    else if ((curSwitch == resetSwitch && LightShow > 0) || (resetDoubleClick && LightShow == 0)) {
      LightShow += 1;
    ...
    } else if ((curSwitch == otherSwitch || anyOtherClick) && LightShow > 0) {
      RPU_TurnOffAllLamps();
      returnState = 20000 + LightShow;
    ...
    } else if (LightShow == 0 && (curSwitch == resetSwitch || (resetBeingHeld && CurrentTime > LastSolTestTime + 250))) {
      LastSolTestTime = CurrentTime;
      CurValue += 1;
    ...
    if (curSwitch == resetSwitch || (resetBeingHeld && CurrentTime > LastSolTestTime + 250)) {
      ...
      else RPU_SetDisplayBlank(4, 108 >> 1);
    ...
    if (CurrentTime - flashTimer < 3000)
      ...
      RPU_SetDisplay(CurDisplay, SavedValue4[CurDisplay], true);
    Enter4Data(curSwitch, resetDoubleClick, resetBeingHeld, curStateChanged, CurrentTime, resetSwitch, otherSwitch, endSwitch);
