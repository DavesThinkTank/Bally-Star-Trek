...
int MatchMode(boolean curStateChanged) {
  ...
  if (curStateChanged) {
    ...
    if (highestScore > HighScore) {
      ...
      if (GameMode == 1) {  // High score info only saved in regular game mode
...
int RunGamePlayMode(int curState, boolean curStateChanged) {
  ...
  if (CurrentScores[CurrentPlayer] >= ScoreAward1 && ScoreAward1 > 0 && ScoreAwardStates[CurrentPlayer] == 0) {  // Mini-Game 1
    ScoreAwardStates[CurrentPlayer] = 1;
    RPU_SetLampState(LA_SAME_PLAYER_SHOOTS_AGAIN, 1);
    RPU_SetLampState(LA_SHOOT_AGAIN, 1);
  }
  if (CurrentScores[CurrentPlayer] >= ScoreAward2 && ScoreAward2 > 0 && ScoreAwardStates[CurrentPlayer] == 1) {  // Mini-Game 2
    ScoreAwardStates[CurrentPlayer] = 2;
    RPU_SetLampState(LA_SAME_PLAYER_SHOOTS_AGAIN, 1);
    RPU_SetLampState(LA_SHOOT_AGAIN, 1);
  }
  if (CurrentScores[CurrentPlayer] >= ScoreAward3 && ScoreAward3 > 0 && ScoreAwardStates[CurrentPlayer] == 2) {  // Mini-Game 3
