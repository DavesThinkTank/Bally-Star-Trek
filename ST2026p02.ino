...
#define VERSION_NUMBER 2026.02       // Version number appears in Display #1 / Credit display at start of game
...
#define KOBAYASHI_MARU_TIME 15       // Time in seconds to disable a Klingon during Kobayashi Maru test

// TELL THE ARDUINO IF YOU HAVE OR DO NOT HAVE MUSIC FILES SET UP ON THE WAV TRIGGER!
// Go to file STyyyypmm.h, near the bottom, around line 525.
// Find the two lines that define BackgroundMusic. One is defined, the other is commented out (i.e., starts with //). One includes the 
// numbers 181 through 187, the other contains only the number 181 repeated. If you have music files numbered 181 through 187 on the WAV Trigger, 
// comment out the line that is all 181, and do not include // at the beginning of the line with 181 through 187. If you do not have these music files,
// comment out the line with numbers 181 through 187, and do not include // at the beginning of the line that is all 181.
// At the bottom of STyyyypmm.h you will find similar definitions for background music for the mini-game challenges. Comment out either the 181 lines, 
// or the others, depending on whether or not you have numbered music files set up on the WAV Trigger.

// See the following YouTube video for more information, as well as instructions for updating this software and transferring it to your Arduino: https://youtu.be/r4PpJglYBh4
...
void SetupDIPVars() {  // Set up all variables derived from the DIP switches
  ...
  dipThresholdAward = dipThresholdAward + (dipThresholdAward < 2);  // 1-2-3 = 25000 - extra ball - free game
...
void setup() {
  ...
  if (RPU_ReadSingleSwitchState(SW_CREDIT_BUTTON)) {  // Turn on Kids' Mode if holding game button
    RPU_ClearSwitches();
    kidMode = true;
    KidSettings();
    SetupAudio(VoiceKidsMode[random(1, 1 + VoiceKidsMode[0])]);  // Play random startup sound
  } else
...
int InitNewBall(bool curStateChanged, int ballNum) {
  ...
  if (!RPU_ReadSingleSwitchState(SW_OUTHOLE)) {  // Make sure ball gets out of outhole before proceeding
    ...
    if (ScoreAwardStates[CurrentPlayer] >= 1 && MiniGamePlayed[CurrentPlayer] == 0) {
      ...
      return MACHINE_STATE_MINI_GAME_3;
    } else
...
int RunAttractMode(int curState, boolean curStateChanged) {
  ...
  if ((CurrentTime - kidTimer > 200) && (kidSwitch != 0xFF)) {                                  // Check if credit button and coin 3 button pressed at same time
    if (RPU_ReadSingleSwitchState(SW_CREDIT_BUTTON) && RPU_ReadSingleSwitchState(SW_COIN_3)) {  // if yes, change kid mode off / on
      kidMode = !kidMode;
      if (!kidMode) {
